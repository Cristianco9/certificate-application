...
/**
 * Controller function to delete an existing academic level.
 *
 * Extracts the academic level id from the request body, delegates the
 * deletion to AcademicLevelServices (which guards against academic
 * levels with associated users), and responds according to the outcome.
 ...
 * @param {Object} req.body - The validated request body (see academicLevelSchema.deleteAcademicLevel).
 * @param {string} req.body.id - The id of the academic level to delete.
 ...
 */
export const deleteOneAcademicLevel = async (req, res, next) => {
  ...
  try {
    // Attempt to delete the academic level
    const response = await academicLevelManager.deleteOne(id);
    ...
    // notFound, conflict when users are still associated); boomify()
    // passes those through untouched and only defaults unexpected
    // errors to a 500.
    const boomError = Boom.boomify(error, {
      message: 'No es posible eliminar el nivel académico de la base de datos',
    });
    next(boomError);
  }
};
